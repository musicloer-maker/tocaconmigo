...
const BARCELONA_ZONES = [
  ...
  { name: 'Eixample', lat: 41.3900, lng: 2.1600 },
  { name: 'Sants-Montjuïc', lat: 41.3700, lng: 2.1400 },
  { name: 'Les Corts', lat: 41.3850, lng: 2.1200 },
  { name: 'Sarrià-Sant Gervasi', lat: 41.4000, lng: 2.1300 },
  { name: 'Gràcia', lat: 41.4050, lng: 2.1550 },
  { name: 'Horta-Guinardó', lat: 41.4200, lng: 2.1650 },
  { name: 'Nou Barris', lat: 41.4400, lng: 2.1750 },
  { name: 'Sant Andreu', lat: 41.4350, lng: 2.1900 },
  { name: 'Sant Martí', lat: 41.4000, lng: 2.2000 },
];
...
export default function ProfilePage() {
  ...
  const loadProfile = async () => {
    ...
    if (instrumentsError || stylesError || lookingForError) {
      setErrorMsg('No hemos podido cargar las opciones musicales.');
      setLoading(false);
      return;
    }

    if (
      profileInstrumentsError ||
      profileStylesError ||
      profileLookingForError
    ) {
      setErrorMsg('No hemos podido cargar tus preferencias musicales.');
      setLoading(false);
      return;
    }
    ...
    setSelectedLookingForIds(
      (profileLookingForData ?? []).map((item) => item.looking_for_id)
    );
  ...
  const toggleLookingFor = (id: number) => {
    setSelectedLookingForIds((current) =>
      current.includes(id)
        ? current.filter((lookingForId) => lookingForId !== id)
        : [...current, id]
    );
  };
...
const { data: profileData, error: profileError } = await supabase
    ...
    const { error: deleteLookingForError } = await supabase
      .from('profile_looking_for')
      .delete()
      .eq('profile_id', user.id);

    if (deleteLookingForError) {
      setErrorMsg('No hemos podido actualizar lo que buscas.');
      setSaving(false);
      return;
    }
    ...
    if (selectedLookingForIds.length > 0) {
      const lookingForRows = selectedLookingForIds.map((lookingForId) => ({
        profile_id: user.id,
      ...
      const { error: insertLookingForError } = await supabase
        .from('profile_looking_for')
        .insert(lookingForRows);

      if (insertLookingForError) {
        setErrorMsg('No hemos podido guardar lo que buscas.');
        setSaving(false);
        return;
      }
    }
